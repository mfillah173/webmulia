...
@section('title', 'Edit Gambar Galeri Fasilitas')
@section('page-title', 'Edit Gambar Galeri Fasilitas')
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    <i class="fas fa-edit me-2"></i>Edit Gambar Galeri Fasilitas
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.galeri-fasilitas.update', $galeriFasilitas) }}" method="POST" enctype="multipart/form-data" id="galeriForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select @error('fasilitas_id') is-invalid @enderror" id="fasilitas_id" name="fasilitas_id" required>
                            ...
                            <option value="{{ $fasilita->id }}" {{ old('fasilitas_id', $galeriFasilitas->fasilitas_id) == $fasilita->id ? 'selected' : '' }}>
                                {{ $fasilita->nama }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                            ...
                            <option value="active" {{ old('status', $galeriFasilitas->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                            <option value="inactive" {{ old('status', $galeriFasilitas->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        <div class="form-help">Status tampil di website. Urutan: {{ $galeriFasilitas->urutan }} (otomatis)</div>
                        @error('status')
                        <div class="invalid-feedback">
                            <i class="fas fa-exclamation-circle me-1"></i>{{ $message }}
                        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="info-item">
                    ...
                    <strong>Fasilitas Terpilih</strong>
                    <p class="text-muted mb-0">{{ $galeriFasilitas->fasilitas->nama ?? 'Fasilitas Tidak Ditemukan' }}</p>
                </div>

                <div class="info-item mt-3">
                    <i class="fas fa-sort text-success me-2"></i>
                    <strong>Urutan</strong>
                    <p class="text-muted mb-0">#{{ $galeriFasilitas->urutan }} (otomatis berdasarkan waktu upload)</p>
                </div>

                <div class="info-item mt-3">
                    <i class="fas fa-calendar text-info me-2"></i>
                    <strong>Dibuat</strong>
                    <p class="text-muted mb-0">{{ $galeriFasilitas->created_at->format('d M Y H:i') }}</p>
                </div>
            </div>
        </div>
    </div>
